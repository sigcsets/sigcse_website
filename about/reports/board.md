---
title: Board Meeting Minutes
layout: page
minutes:

  - board:
    year: "2025 to 2028 Board"
    folder: "BoardMinutes/2025-2028/"
    approved:
      - description: "Minutes of the SIGCSE Board Meeting May 15, 2026"
        date: 2026-05-15

      - description: "Minutes of the SIGCSE Board Meeting Apr 22, 2026"
        date: 2026-04-22

      - description: "Minutes of the SIGCSE Board Meeting Mar 18, 2026"
        date: 2026-03-18

      - description: "Minutes of the SIGCSE Board Open Business Meeting Feb 20, 2026"
        date: 2026-02-20

      - description: "Minutes of the SIGCSE Board TS Retreat Feb 18, 2026"
        date: 2026-02-18

      - description: "Minutes of the SIGCSE Board Meeting Jan 23, 2026"
        date: 2026-01-23

      - description: "Minutes of the SIGCSE Board Meeting Dec 17, 2025"
        date: 2025-12-17

      - description: "Minutes of the SIGCSE Board Meeting Nov 11, 2025"
        date: 2025-11-19

      - description: "Minutes of the SIGCSE Board Meeting Oct 29, 2025"
        date: 2025-10-29

      - description: "Minutes of the SIGCSE Board Retreat Sep 21, 2025"
        date: 2025-09-21

      - description: "Minutes of the SIGCSE Board Meeting Aug 12, 2025"
        date: 2025-08-12

      - description: "Minutes of the SIGCSE Board Meeting Jul 10, 2025"
        date: 2025-07-10

  - board:
    year: "2022 to 2025 Board"
    folder: "BoardMinutes/2022-2025/"
    approved:
      - description: "Minutes of a SIGCSE Board meeting May 20, 2025"
        date: 2025-05-20

      - description: "Minutes of a SIGCSE Board meeting April 4, 2024"
        date: 2025-04-24

      - description: "Minutes of a SIGCSE Board meeting Mar 27, 2025"
        date: 2025-03-27

      - description: "Minutes of a SIGCSE Board meeting Jan 26, 2025"
        date: 2025-02-26

      - description: "Minutes of a SIGCSE Board meeting Jan 23, 2025"
        date: 2025-01-23

      - description: "Minutes of a SIGCSE Board meeting Nov 26, 2024"
        date: 2024-11-26

      - description: "Minutes of a SIGCSE Board meeting Oct 28-29"
        date: 2024-10-28

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-09-17

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-08-21

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-05-22

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-04-24

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-03-20

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-02-28

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2024-01-24

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-12-20

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-10-24

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-09-27

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-08-30

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-08-18

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-08-30

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-07-20

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-06-01

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-04-18

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-03-15

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2022-07-18

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2022-08-16

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2022-09-20

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2022-10-18

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2022-11-22

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-01-24

      - description: "Minutes of a SIGCSE Board meeting"
        date: 2023-02-21

  - board: 
    year: "2019 to 2022 Board"
    folder: "BoardMinutes/2019-2022/"
    approved: 
      - description: Minutes of a SIGCSE Board conference call
        date: 2019-08-05

      - description: "Minutes of a SIGCSE Board meeting in Chicago, IL, USA"
        date: 2019-11-02

      - description: "Minutes of a SIGCSE Board conference call"
        date: 2020-01-16

      - description: "Minutes of a SIGCSE Board meeting in Portland, OR, USA"
        date: 2020-03-11

      - description: "Minutes of a SIGCSE Board conference call"
        date: 2020-03-16

      - description: "Minutes of a SIGCSE Board conference call"
        date: 2020-03-18

      - description: "Minutes of a SIGCSE Board conference call"
        date: 2020-05-04

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-08-25

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-09-22

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-10-20

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-10-27

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-11-17

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2020-12-15

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-02-03

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-02-10

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-02-22

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-04-20

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-04-26

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-05-11

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-05-19

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-06-09

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-06-15

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-07-13

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-08-03

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-09-08

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-09-29

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-10-20

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2021-11-16

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-01-12

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-01-19

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-01-26

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-02-08

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-03-02

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-03-16

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-03-23

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-04-11

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-04-25

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-05-16

      - description: "Minutes of a SIGCSE Board Meeting"
        date: 2022-06-22

  - board: 
    year: "2016 to 2019 Board"
    folder: "BoardMinutes/2016-2019/"
    approved: 
      - date: 2016-08-16
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2016-10-22
        description: "Minutes of a SIGCSE Board meeting in Chicago, IL, USA"
      - date: 2017-01-13
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2017-03-08
        description: "Minutes of a SIGCSE Board meeting in Seattle, WA, USA"
      - date: 2017-05-30
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2017-08-14
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2017-10-22
        description: "Minutes of a SIGCSE Board meeting in Miami, FL, USA"
      - date: 2017-12-12
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2018-01-23
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2018-02-21
        description: "Minutes of a SIGCSE Board meeting in Baltimore, MD, USA"
      - date: 2018-04-26
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2018-06-19
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2018-08-23
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2018-10-27
        description: "Minutes of a SIGCSE Board meeting in Chicago, IL, USA"
      - date: 2019-01-10
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2019-02-27
        description: "Minutes of a SIGCSE Board meeting in Minneapolis, MN, USA"
      - date: 2019-04-25
        description: "Minutes of a SIGCSE Board conference call"

  - board: 
    year: "2013 to 2016 Board"
    folder: "BoardMinutes/2013-2016/"
    approved: 
      - date: 2013-08-21
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2013-10-26
        description: "Minutes of the SIGCSE Board meeting in Chicago, IL, USA"
      - date: 2014-01-13
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2014-03-05
        description: "Minutes of a SIGCSE Board meeting in Atlanta, GA, USA"
      - date: 2014-04-29
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2014-06-13
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2014-08-18
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2014-10-19
        description: "Minutes of the SIGCSE Board meeting in Atlanta, GA, USA"
      - date: 2015-01-14
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2015-03-04
        description: "Minutes of the SIGCSE Board meeting in Kansas City, MO, USA"
      - date: 2015-04-28
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2015-05-26
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2015-09-02
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2015-10-24
        description: "Minutes of the SIGCSE Board meeting in Minneapolis, MN, USA"
      - date: 2015-12-15
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2016-01-11
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2016-01-27
        description: "Minutes of the SIGCSE Board conference call"
      - date: 2016-03-02
        description: "Minutes of the SIGCSE Board meeting in Memphis, TN, USA"
      - date: 2016-04-26
        description: "Minutes of the SIGCSE Board Conference Call"

  - board: 
    year: "2010 to 2013 Board"
    folder: "BoardMinutes/2010-2013/"
    approved: 
      - date: 2010-10-02
        description: "Minutes of the SIGCSE Board meeting in Atlanta, GA, USA"
      - date: 2010-11-30
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2011-01-11
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2011-03-09
        description: "Minutes of the SIGCSE Board meeting in Dallas, TX, USA"
      - date: 2011-04-13
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2011-08-04
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2011-10-15
        description: "Minutes of the SIGCSE Board meeting in Charlotte, NC, USA"
      - date: 2012-01-31
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2012-02-29
        description: "Minutes of the SIGCSE Board meeting in Raleigh, NC, USA"
      - date: 2012-05-29
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2012-09-18
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2012-11-03
        description: "Minutes of the SIGCSE Board meeting in Chicago, IL, USA"
      - date: 2013-01-13
        description: "Minutes of the SIGCSE Board meeting in Denver, CO, USA"
      - date: 2013-03-06
        description: "Minutes of the SIGCSE Board meeting in Denver, CO, USA"

  - board: 
    year: "2007 to 2010 Board"
    folder: "BoardMinutes/2007-2010/"
    approved: 
      - date: 2007-09-14
        description: "Minutes of the SIGCSE Board meeting in Atlanta, GA, USA"
      - date: 2008-03-08
        description: "Minutes of the SIGCSE Board meeting in Portland, OR, USA"
      - date: 2008-10-04
        description: "Minutes of the SIGCSE Board meeting in Keystone, CO, USA"
      - date: 2008-12-05
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2009-01-30
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2009-03-04
        description: "Minutes of the SIGCSE Board Meeting in Chattanooga, TN, USA"
      - date: 2009-04-17
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2009-10-17
        description: "Minutes of the SIGCSE Board meeting in Newark, NJ, USA"
      - date: 2010-03-10
        description: "Minutes of the SIGCSE Board meeting in Milwaukee, WI, USA"
      - date: 2010-05-06
        description: "Minutes of a SIGCSE Board conference call"
      - date: 2010-05-26
        description: "Minutes of a SIGCSE Board conference call"

---

This page contains the minutes of the Board Meetings and occasional conference calls. They are posted here once approved by the board, typically at the following meeting.

{% comment %}
The file is built from the date as "Minutes-YYYY-MM-DD.pdf"
Try iterating over files stored in 
 about/reports/BoardMinutes/yyyy-yyyy/Meeting-yyyy-mm-dd.pdf
 and eliminate all of the info above.
{% for folder in site.about.reports.BoardMinutes %}
* {{folder}}
{% endfor %}
{% endcomment %}

{% assign boards = page.minutes | sort: "year" | reverse %}
{% for board in boards %}
#### {{board.year}}
{% assign meetings = board.approved | sort: "date" | reverse %}
<ul>{% for minute in meetings %}
{% assign url = board.folder | append: "Minutes-" | append: minute.date | append: ".pdf" %}
<li><a href="{{ url }}">{{minute.date | date: '%b %-d, %Y'}}</a> - {{minute.description}}</li>
{% endfor %}</ul>
{% endfor %}


