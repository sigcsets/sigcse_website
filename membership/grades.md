---
title: ACM Advanced Membership Grades
layout: page

members:
- name: William Richards Adrion
  grade: ACM Fellow
- name: Robert M Aiken
  grade: ACM Fellow
- name: Ronald M Baeker
  grade: ACM Fellow
- name: Tracy Camp
  grade: ACM Fellow
- name: Robert S Cartwright
  grade: ACM Fellow
- name: Jack Davidson
  grade: ACM Fellow
- name: Peter J Denning
  grade: ACM Fellow
- name: Dennis J. Frailey
  grade: ACM Fellow
- name: Frank L Friedman
  grade: ACM Fellow
- name: Juan E. Gilbert
  grade: ACM Fellow
- name: Mark Guzdial
  grade: ACM Fellow
- name: Richard E. Ladner
  grade: ACM Fellow
- name: Joyce Currie Little
  grade: ACM Fellow
- name: Michael Littman
  grade: ACM Fellow
- name: Dianne Martin
  grade: ACM Fellow
- name: William B Poucher
  grade: ACM Fellow
- name: John Mark Pullen
  grade: ACM Fellow
- name: Eric S Roberts
  grade: ACM Fellow
- name: Bhavani Thuraisingham
  grade: ACM Fellow
- name: Allen Tucker
  grade: ACM Fellow
- name: Albert J Turner
  grade: ACM Fellow
- name: Stuart Zweben
  grade: ACM Fellow
- name: Joel Adams
  grade: Distinguished Educator
- name: Christine Alvarado
  grade: Distinguished Educator
- name: Owen Astrachan
  grade: Distinguished Educator
- name: Valerie Barr
  grade: Distinguished Member
- name: Andrew B. Begel
  grade: Distinguished Member
- name: Tim Bell
  grade: Distinguished Educator
- name: Mordechai Ben-Ari
  grade: Distinguished Educator
- name: Judith Bishop
  grade: Distinguished Educator
- name: Kim B Bruce
  grade: Distinguished Member
- name: Martin C Carlisle
  grade: Distinguished Educator
- name: Michael E. Caspersen
  grade: Distinguished Educator
- name: Lillian (Boots) Cassel
  grade: Distinguished Educator
- name: Gail Chapman
  grade: Distinguished Member
- name: Michael J Clancy
  grade: Distinguished Educator
- name: Steve Cooper
  grade: Distinguished Educator
- name: Thomas Cortina
  grade: Distinguished Member
- name: Michelle Craig
  grade: Distinguished Educator
- name: James H. Cross II
  grade: Distinguished Member
- name: Mats Daniels
  grade: Distinguished Scientist
- name: Andrea Danyluk
  grade: Distinguished Member
- name: Marie DesJardins
  grade: Distinguished Scientist
- name: Suzanne W Dietrich
  grade: Distinguished Educator
- name: Laura Dillon
  grade: Distinguished Scientist
- name: Stephen H Edwards
  grade: Distinguished Educator
- name: Alan D Fekete
  grade: Distinguished Scientist
- name: Sally Fincher
  grade: Distinguished Scientist
- name: Armando Fox
  grade: Distinguished Scientist
- name: Dan Garcia
  grade: Distinguished Educator
- name: Jeff Gray
  grade: Distinguished Educator
- name: Elizabeth K Hawthorne
  grade: Distinguished Educator
- name: Lane A Hemaspaandra
  grade: Distinguished Scientist
- name: Cay S. Horstmann
  grade: Distinguished Member
- name: Matt Huenerfauth
  grade: Distinguished Member
- name: John Impagliazzo
  grade: Distinguished Educator
- name: Doug Jacobson
  grade: Distinguished Educator
- name: Charles F. Kelemen
  grade: Distinguished Educator
- name: Michael Kolling
  grade: Distinguished Educator
- name: Renee McCauley
  grade: Distinguished Member
- name: Andrew McGettrick
  grade: Distinguished Member
- name: Ethan L Miller
  grade: Distinguished Scientist
- name: Barbara Boucher Owens
  grade: Distinguished Educator
- name: Richard Pattis
  grade: Distinguished Educator
- name: Manuel Pérez-Quiñones
  grade: Distinguished Member
- name: Marian Petre
  grade: Distinguished Scientist
- name: Sushil K Prasad
  grade: Distinguished Scientist
- name: Ramon C Puigjaner
  grade: Distinguished Educator
- name: Susan Rodger
  grade: Distinguished Educator
- name: Mehran Sahami
  grade: Distinguished Member
- name: Amber Settle
  grade: Distinguished Member
- name: Clifford A Shaffer
  grade: Distinguished Educator
- name: Judithe Sheard
  grade: Distinguished Member
- name: Lynn Andrea Stein
  grade: Distinguished Member
- name: Chris Stephenson
  grade: Distinguished Member
- name: Jodi L Tims
  grade: Distinguished Member
- name: David S Touretzky
  grade: Distinguished Scientist
- name: Andre van der Hoek
  grade: Distinguished Member
- name: Henry M Walker
  grade: Distinguished Educator
- name: Robert A. Walker
  grade: Distinguished Scientist
- name: Kevin D Wayne
  grade: Distinguished Educator
- name: Charles Weems
  grade: Distinguished Member
- name: Mark Weiss
  grade: Distinguished Educator
- name: Rajeev Agrawal
  grade: ACM Senior Member
- name: Vicki Almstrum
  grade: ACM Senior Member
- name: Barbara Anthony
  grade: ACM Senior Member
- name: David Arnow
  grade: ACM Senior Member
- name: Reyyan Ayfer
  grade: ACM Senior Member
- name: Douglas Baldwin
  grade: ACM Senior Member
- name: Valerie Barr
  grade: ACM Senior Member
- name: Rebecca A Bates
  grade: ACM Senior Member
- name: Joseph A Bergin
  grade: ACM Senior Member
- name: Cindy Bethel
  grade: ACM Senior Member
- name: Benjamin Bishop
  grade: ACM Senior Member
- name: Jean Blair
  grade: ACM Senior Member
- name: Richard Blumenthal
  grade: ACM Senior Member
- name: Erik Brunvand
  grade: ACM Senior Member
- name: Peter Brusilovsky
  grade: ACM Senior Member
- name: Suzanne Buchele
  grade: ACM Senior Member
- name: Jennifer Campbell
  grade: ACM Senior Member
- name: Jeffrey C Carver
  grade: ACM Senior Member
- name: Vincent A Cicirello
  grade: ACM Senior Member
- name: John Cigas
  grade: ACM Senior Member
- name: Alison Clear
  grade: ACM Senior Member
- name: Thomas Cortina
  grade: ACM Senior Member
- name: Venu G Dasigi
  grade: ACM Senior Member
- name: Janet Davis
  grade: ACM Senior Member
- name: Karen Davis
  grade: ACM Senior Member
- name: Susan Dean
  grade: ACM Senior Member
- name: Adrienne Decker
  grade: ACM Senior Member
- name: John Dooley
  grade: ACM Senior Member
- name: John P Dougherty
  grade: ACM Senior Member
- name: J.Philip East
  grade: ACM Senior Member
- name: Ali Ebnenasir
  grade: ACM Senior Member
- name: Deger Erdil
  grade: ACM Senior Member
- name: Barbara Jane Ericson
  grade: ACM Senior Member
- name: Vladimir Estivill-Castro
  grade: ACM Senior Member
- name: Rudolf Fleischer
  grade: ACM Senior Member
- name: Yetunde Folajimi
  grade: ACM Senior Member
- name: Timothy Fossum
  grade: ACM Senior Member
- name: Edward F Gehringer
  grade: ACM Senior Member
- name: Paul V. Gestwicki
  grade: ACM Senior Member
- name: Don Goelman
  grade: ACM Senior Member
- name: Timothy A Gonsalves
  grade: ACM Senior Member
- name: Rebecca Grasser
  grade: ACM Senior Member
- name: James Heliotis
  grade: ACM Senior Member
- name: Daryl Hepting
  grade: ACM Senior Member
- name: Mark A Holliday
  grade: ACM Senior Member
- name: Ellis Horowitz
  grade: ACM Senior Member
- name: Meriel Huggard
  grade: ACM Senior Member
- name: Hongbo Jiang
  grade: ACM Senior Member
- name: Jeremy R Johnson
  grade: ACM Senior Member
- name: Lawrence G Jones
  grade: ACM Senior Member
- name: Daniel T Joyce
  grade: ACM Senior Member
- name: Jennifer S. Kay
  grade: ACM Senior Member
- name: Carsten Kleiner
  grade: ACM Senior Member
- name: Vladik Kreinovich
  grade: ACM Senior Member
- name: Can Kultur
  grade: ACM Senior Member
- name: Deepak Kumar
  grade: ACM Senior Member
- name: Clif Kussmaul
  grade: ACM Senior Member
- name: David Largent
  grade: ACM Senior Member
- name: Niels Muller Larsen
  grade: ACM Senior Member
- name: Cary Laxer
  grade: ACM Senior Member
- name: Robert R. Lewis
  grade: ACM Senior Member
- name: Fuhua (Oscar) Lin
  grade: ACM Senior Member
- name: Erik Linstead
  grade: ACM Senior Member
- name: Vicente Ferreira de Lucena
  grade: ACM Senior Member
- name: David Luginbuhl
  grade: ACM Senior Member
- name: Roman Lysecky
  grade: ACM Senior Member
- name: Cynthia R Marling
  grade: ACM Senior Member
- name: Bruce R Maxim
  grade: ACM Senior Member
- name: Ciaran Mc Goldrick
  grade: ACM Senior Member
- name: John W McCormick
  grade: ACM Senior Member
- name: Kevin W McCullen
  grade: ACM Senior Member
- name: Monica McGill
  grade: ACM Senior Member
- name: James McGuffee
  grade: ACM Senior Member
- name: James Miller
  grade: ACM Senior Member
- name: Antonija Mitrovic
  grade: ACM Senior Member
- name: William Mongan
  grade: ACM Senior Member
- name: Freeman L Moore
  grade: ACM Senior Member
- name: Michael O Moorman
  grade: ACM Senior Member
- name: Patricia A Morreale
  grade: ACM Senior Member
- name: Todd Neller
  grade: ACM Senior Member
- name: Amos Olagunju
  grade: ACM Senior Member
- name: Joann Ordille
  grade: ACM Senior Member
- name: Michael Oudshoorn
  grade: ACM Senior Member
- name: Panagiotis Papadimitratos
  grade: ACM Senior Member
- name: Abelardo Pardo
  grade: ACM Senior Member
- name: Manuel A. Pérez-Quiñones
  grade: ACM Senior Member
- name: Jeffrey Popyack
  grade: ACM Senior Member
- name: Leo Porter
  grade: ACM Senior Member
- name: Viera Krnanova Proulx
  grade: ACM Senior Member
- name: Samuel A. Rebelsky
  grade: ACM Senior Member
- name: Donna S Reese
  grade: ACM Senior Member
- name: Stefan A Robila
  grade: ACM Senior Member
- name: Guido Roessling
  grade: ACM Senior Member
- name: Anthony S Ruocco
  grade: ACM Senior Member
- name: Amber Settle
  grade: ACM Senior Member
- name: Kelly Shaw
  grade: ACM Senior Member
- name: Richard E Smith
  grade: ACM Senior Member
- name: Peter Sommerlad
  grade: ACM Senior Member
- name: Daniel R. Tauritz
  grade: ACM Senior Member
- name: Eli Tilevich
  grade: ACM Senior Member
- name: Jodi Tims
  grade: ACM Senior Member
- name: Kim Tracy
  grade: ACM Senior Member
- name: Paul Tymann
  grade: ACM Senior Member
- name: Jan Vahrenhold
  grade: ACM Senior Member
- name: Michael Van Hilst
  grade: ACM Senior Member
- name: Troy Vasiga
  grade: ACM Senior Member
- name: Kam H Vat
  grade: ACM Senior Member
- name: J. Angel Velazquez-Iturbide
  grade: ACM Senior Member
- name: Ellen L Walker
  grade: ACM Senior Member
- name: Jerod Weinman
  grade: ACM Senior Member
- name: James Whitmore
  grade: ACM Senior Member
- name: Gunnar Wolf
  grade: ACM Senior Member
- name: Steven A Wolfman
  grade: ACM Senior Member
- name: Xiao-Chun Yang
  grade: ACM Senior Member
- name: Mohamed M Zahran
  grade: ACM Senior Member
- name: Tiffany Barnes
  grade: Distinguished Educator
  year: 2020
- name: Barb Ericson
  grade: Distinguished Educator
  year: 2020
- name: Deepak Kumar
  grade: Distinguished Educator
  year: 2020
- name: Panagiotis Papadimitratos
  grade: Distinguished Scientist
  year: 2020
- name: Gloria Childress Townsend
  grade: Distinguished Educator
  year: 2020

---

Last updated December, 2019

The Awards Committee of the SIGCSE Board is interested in raising the profile of computing education, and we believe that one way to do this is to have more SIGCSE members identified as Advanced Members of the ACM. There are three advanced membership grades available: Senior, Distinguished, and Fellow. Requirements, due dates, nomination forms and FAQs about each of these grades are available at the [ACM Awards page](http://awards.acm.org/).

A very useful reference is an article by Marc Snir & Telle Whitney in the July 2010 CACM: [Advice to members seeking ACM distinction](http://portal.acm.org/citation.cfm?id=1785414.1785432&coll=portal&dl=ACM&preflayout=flat).

Feel free to contact [Amber Settle](mailto:asettle@cdm.depaul.edu) if you have questions.


#### ACM Fellow
{:.border-bottom}

This is ACM's most prestigious member grade recognizing the top 1% of ACM members for their outstanding accomplishments in computing and information technology and/or outstanding service to ACM and the larger computing community. Applications are due annually at the beginning of September. Fellows require five endorsements from current ACM Professional Members and must be nominated by an ACM Professional Member.

###### SIGCSE members who are ACM Fellows

<table class="table">
  <tr>
{% assign fellows = page.members | where: "grade", "ACM Fellow" %}
{% assign count = 1 %}
{% for fellow in fellows %}
  <td>{{fellow.name}}</td>
  {% if count == 3 %}</tr><tr>
    {% assign count = 1 %}
  {%else%}
    {% assign count = count | plus:1 %}
  {%endif%}
{%endfor%}
</tr>
</table>

<p>&nbsp;</p>

#### ACM Distinguished Member
{:.mt4 .border-bottom}

This membership grade recognizes members with exemplary achievements that have advanced the science, engineering and education of computing. Applications are due annually on August 1.

This award level requires at least 15 years of professional experience and the ability to demonstrate that the member has had significant accomplishments or impact in the computing field. Distinguished Members must be nominated by a current ACM Professional Member.


###### SIGCSE members: Distinguished Educator status


<table class="table">
  <tr>
{% assign fellows = page.members | where: "grade", "Distinguished Educator" %}
{% assign count = 1 %}
{% for fellow in fellows %}
  <td>{{fellow.name}}</td>
  {% if count == 3 %}</tr><tr>
    {% assign count = 1 %}
  {%else%}
    {% assign count = count | plus:1 %}
  {%endif%}
{%endfor%}
</tr>
</table>

###### SIGCSE members: Distinguished Scientist status

<table class="table">
  <tr>
{% assign fellows = page.members | where: "grade", "Distinguished Scientist" %}
{% assign count = 1 %}
{% for fellow in fellows %}
  <td>{{fellow.name}}</td>
  {% if count == 3 %}</tr><tr>
    {% assign count = 1 %}
  {%else%}
    {% assign count = count | plus:1 %}
  {%endif%}
{%endfor%}
</tr>
</table>

###### SIGCSE members: Distinguished Member status

<table class="table">
  <tr>
{% assign fellows = page.members | where: "grade", "Distinguished Member" %}
{% assign count = 1 %}
{% for fellow in fellows %}
  <td>{{fellow.name}}</td>
  {% if count == 3 %}</tr><tr>
    {% assign count = 1 %}
  {%else%}
    {% assign count = count | plus:1 %}
  {%endif%}
{%endfor%}
</tr>
</table>


<p>&nbsp;</p>

#### Senior Member
{:.mt4 .border-bottom}

ACM members can apply for the first advanced membership grade, Senior Membership, approximately every three months. Senior Membership requires at least 10 years of professional experience, including graduate school, and 5 years of continuous Professional Membership. Senior members are self-nominating, and three letters of recommendation from colleagues are required.

###### SIGCSE members: ACM Senior Member status

<table class="table">
  <tr>
{% assign fellows = page.members | where: "grade", "ACM Senior Member" %}
{% assign count = 1 %}
{% for fellow in fellows %}
  <td>{{fellow.name}}</td>
  {% if count == 3 %}</tr><tr>
    {% assign count = 1 %}
  {%else%}
    {% assign count = count | plus:1 %}
  {%endif%}
{%endfor%}
</tr>
</table>


